...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($uploadedPhoto) {
        ...
        $stmt = $conn->prepare(
            "UPDATE featured_products SET product_category = ?, product_name = ?, product_description = ?, product_picture= ? WHERE id = ?"
        );
        $stmt->bind_param(
            "ssssi",
            $product_category,
        ...
        $stmt = $conn->prepare(
            "UPDATE featured_products SET product_category = ?, product_name = ?, product_description = ? WHERE id = ?"
        );
        $stmt->bind_param(
            "sssi",
            $department,
    ...
    if ($stmt->execute()) {
        ...
        header("Location: ../dashboard.php?success&type=featured_categoryUpS");
        exit();
    } else {
        // Redirect with error flag
        header("Location: ../dashboard.php?error&type=featured_catogoryUpF");
        exit();
    }
